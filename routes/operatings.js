...
router.get('/lastOperatings',verifyToken,(req,res)=>{
    Operatings.find({operatingType:1},(err,find_operating)=>{
        if(err){
            return res.render('error.ejs'); 
        }
    ...
    }).sort({'find_operating.operatingValidationCreated': -1}).limit(50);
});
...
router.get('/deleteOperating/:_id',verifyToken,(req,res)=>{
    Operatings.findOneAndRemove({_id:req.params._id},(err,find_operating)=>{
        if(err){
            return res.render('error.ejs');
        }
